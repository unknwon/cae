...
func TestPackTo(t *testing.T) {
	Convey("Pack a dir or file to tar.gz file", t, func() {
		Convey("Pack a dir that does exist and includir root dir", func() {
			err := PackTo("testdata/testdir", path.Join(os.TempDir(), "testdata/testdir1.tar.gz"), true)
			So(err, ShouldBeNil)
		})

		Convey("Pack a dir that does exist and does not includir root dir", func() {
			err := PackTo("testdata/testdir", path.Join(os.TempDir(), "testdata/testdir2.tar.gz"))
			So(err, ShouldBeNil)
		})

		Convey("Pack a dir that does not exist and does not includir root dir", func() {
			err := PackTo("testdata/testdir404", path.Join(os.TempDir(), "testdata/testdir3.tar.gz"))
			So(err, ShouldNotBeNil)
		})

		Convey("Pack a file that does exist", func() {
			err := PackTo("testdata/README.txt", path.Join(os.TempDir(), "testdata/testdir4.tar.gz"))
			So(err, ShouldBeNil)
		})

		Convey("Pack a file that does not exist", func() {
			err := PackTo("testdata/README404.txt", path.Join(os.TempDir(), "testdata/testdir5.tar.gz"))
			So(err, ShouldNotBeNil)
		})
	})
}
